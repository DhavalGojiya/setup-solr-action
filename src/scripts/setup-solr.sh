...
# -----------------------------------------
echo "🚀 Starting Solr container on port $SOLR_HOST_PORT..."
docker run -d \
...
if [ -n "$SOLR_CUSTOM_CONFIGSET_PATH" ]; then
    echo "📂 Copying configs from '$SOLR_CUSTOM_CONFIGSET_PATH' to core '$SOLR_CORE_NAME'..."
    ...
    echo "⚠️  Skipping custom configs: path is empty or does not exist."
fi
...
echo "🔄 Reloading Solr core at '$SOLR_CORE_RELOAD_URL' ⏳..."
...
if [ "$HTTP_CODE" -eq 200 ]; then
    echo "✅ Solr core reloaded successfully (HTTP $HTTP_CODE)"
else
    echo "❌ Failed to reload Solr core (HTTP $HTTP_CODE)"
    echo "$BODY"
    exit 1
fi
